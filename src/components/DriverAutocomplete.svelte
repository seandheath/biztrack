<!--
  Driver text input with autocomplete dropdown from driverCache.
...
-->
<script>
  import { driverCache } from '$lib/store.js';
  ...
  let listboxId = $derived(id ? `${id}-listbox` : 'driver-listbox');
...
<div class="relative">
  ...
  {#if open && suggestions.length > 0}
    <ul
      ...
      role="listbox"
    >
      {#each suggestions as driver, i (driver)}
        <li role="option" aria-selected={i === activeIdx}>
          <button
            ...
            "
            onpointerdown={(e) => { e.preventDefault(); pick(driver); }}
          >
            {driver}
          </button>
        </li>
